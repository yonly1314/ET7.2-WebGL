...
namespace ET
{
    ...
    public class NetServices: Singleton<NetServices>
    {
        ...
        {
            NetOperator netOperator = new NetOperator() { Op = NetOp.ChangeAddress, ServiceId = serviceId, ChannelId = channelId, Object =
 ipEndPoint};
            this.netThreadOperators.Enqueue(netOperator);
        }
#else
        public async ETTask<(uint, uint)> GetChannelConn(int serviceId, long channelId)
        {
            ETTask<(uint, uint)> tcs = ETTask<(uint, uint)>.Create();
            NetOperator netOperator = new NetOperator() { Op = NetOp.GetChannelConn, ServiceId = serviceId, ChannelId = channelId, Object = tcs };
            this.netThreadOperators.Enqueue(netOperator);
            return await tcs;
        }

        public void ChangeAddress(int serviceId, long channelId, string ipEndPoint)
        {
...
#else
        public void CreateChannel(int serviceId, long channelId, string address)
        {
            NetOperator netOperator = new NetOperator() { Op = NetOp.CreateChannel, ServiceId = serviceId, ChannelId = channelId, Object = address };
            this.netThreadOperators.Enqueue(netOperator);
        }
...
#else
                                service.Create(op.ChannelId, op.Object as string);
#endif
                            }
...
#else
                            var tcs = op.Object as ETTask<ValueTuple<uint, uint>>;
#endif
                            try
                            {
                                AService service = this.Get(op.ServiceId);
...
#else
                            service.ChangeAddress(op.ChannelId, op.Object as string);
#endif
                            break;
                        }
                        default:
