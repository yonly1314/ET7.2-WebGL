...
namespace ET
{
    public class WChannel: AChannel
    {
        ...
        public WChannel(long id, WebSocket webSocket, WService service)
        {
            this.Id = id;
            this.Service = service;
            this.ChannelType = ChannelType.Connect;
        ...
        public async ETTask ConnectAsync()
        {
            try
            {
                ...
                this.webSocket.OnOpen += (sender, args) => { tcs.SetResult();};
                this.webSocket.OnMessage += (sender, args) =>
                {
                    if (args.RawData.Length > ushort.MaxValue)
                    {
                        this.webSocket.CloseAsync();
                        this.OnError(ErrorCore.ERR_WebsocketMessageTooBig);
                        return;
                    ...
                    this.recvStream.SetLength(args.RawData.Length);
                    this.recvStream.Seek(2, SeekOrigin.Begin);
                    Array.Copy(args.RawData, 0, this.recvStream.GetBuffer(), 0, args.RawData.Length);
                    this.OnRead(this.recvStream);
                };
        ...
        public async ETTask StartSend()
        {
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    try
                    {
                        var memory = new ReadOnlyMemory<byte>(bytes.GetBuffer(), (int)bytes.Position, (int)(bytes.Length - bytes.Position));
                        this.webSocket.SendAsync(memory.ToArray());
