...
#else
        [ObjectSystem]
        public class AwakeSystem: AwakeSystem<NetClientComponent>
        {
            protected override void Awake(NetClientComponent self)
            {
...
#else
        public static Session Create(this NetClientComponent self, string address)
        {
            long channelId = NetServices.Instance.CreateConnectChannelId();
            Session session = self.AddChildWithId<Session, int>(channelId, self.ServiceId);
            ...
            NetServices.Instance.CreateChannel(self.ServiceId, session.Id, address);
