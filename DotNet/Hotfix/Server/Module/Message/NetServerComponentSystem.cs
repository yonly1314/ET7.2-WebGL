...
namespace ET.Server
{
    ...
    public static class NetServerComponentSystem
    {
        ...
        [ObjectSystem]
        public class Awake1System: AwakeSystem<NetServerComponent, string>
        {
            protected override void Awake(NetServerComponent self, string address)
            {
                self.ServiceId = NetServices.Instance.AddService(new WService(new []{address}));
                NetServices.Instance.RegisterAcceptCallback(self.ServiceId, self.OnAccept);
                NetServices.Instance.RegisterReadCallback(self.ServiceId, self.OnRead);
                NetServices.Instance.RegisterErrorCallback(self.ServiceId, self.OnError);
            }
